...
def transform_ai_response(document: documentai.Document) -> dict:
    ...
    """
    # The AI sometimes finds more fields than we need, so we specify all the 
    # columns we want to keep, ensuring no unexpected fields are added.
    BQ_FLAT_COLUMNS = {
    ...
    # Start by creating a dictionary with only the flat fields we care about.
    row_to_insert = {
    ...
    # Then, parse the nested properties for line_items and vat into a list of items.
    parsed_line_items = []
    parsed_vat = []
    for entity in document.entities:
        key = entity.type_
